...
namespace PadControlCenter
{
    class DynamicServer
    {
        ...
        public void Main()
        {
            ...
            string pageDir = GlobalConf.getconf_aspath("dynamic", "pagedir");
            ...
            string pagePath = Path.Combine(pageDir, mainPage + ".uipage");
            try
            {
                _modeContent = File.ReadAllText(pagePath);
            }
            catch(Exception e)
            {
                Logger.Error($"{e}");
                Logger.Info("Press any key for exit.");
                Console.ReadKey();
                return;
            }
            ...
            _server.Start(port, pageDir, null);
