...
namespace SmartTrafficCtrl
{
    public class MainControl
    {
        ...
        public void StartUp()   // 程序启动
        {
            ...
            SerialPortMgr.Instance._procol.SetOnMeetBlock(DoMeetBlock);
            ...
            switch(command.cmd)
            {
                case "set_car":
                    if(command.getarg<int>(0, out int robotIndex))
                    {
                        ...
                        SerialPortMgr.Instance.SetCurCar(robotIndex);
                ...
                case "set_truck":
                    if (command.getarg<int>(0, out int truckIndex))
                    {
                        ...
                        SerialPortMgr.Instance.SetCurTruck(truckIndex);
                ...
                case "go_continue":
                    Logger.Info($"继续行驶");
                    SerialPortMgr.Instance.SendGo();
                    property.ClearBlock();
                    break;

                case "start_truck":
                    Logger.Info($"货车启动");
                    SerialPortMgr.Instance.TruckStart();
                    property.SetUseTruck(true);
                    break;

                case "stop_truck":
                    Logger.Info($"货车停止");
                    SerialPortMgr.Instance.TruckStop();
