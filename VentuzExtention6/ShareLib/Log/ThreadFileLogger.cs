...
namespace ShareLib.Log
{
    public class ThreadFileLogger : FileLogger
    {
        ...
        {
            return $"{ AppDomain.CurrentDomain.BaseDirectory }Log\\{ DateTime.Now.ToString("yyyy-MM-dd") }.{ Process.GetCurrentProcess().Id }.{ Thread.CurrentThread.ManagedThreadId }.log";
        }
    }
}
